[**TDLib**](../../../../../../README.md) • **Docs**

***

[TDLib](../../../../../../modules.md) / [index](../../../../../README.md) / [types](../../../README.md) / [default](../README.md) / ReactionNotificationSettings

# Type Alias: ReactionNotificationSettings

> **ReactionNotificationSettings**: [`reactionNotificationSettings`](reactionNotificationSettings-1.md)

Any of:
- [reactionNotificationSettings](reactionNotificationSettings-1.md)

## Defined in

dist/generated/types.d.ts:60811
